...
class Experiment(object):
    ...
    def prepare(self):
        ...
        # print(self.experiment_parameter.get(self.experiment_parameter.PCAP))
        if self.experiment_parameter.get(self.experiment_parameter.PCAP) == 'yes':
            self.run_tcpdump()
        self.run_netem_at()
    ...
    def clean(self):
        ...
        method, while still calling this parent function.
        """
        for client in self.topo_config.clients:
            self.topo.command_to(
                client, "killall tcpdump")
    ...
    def run_tcpdump(self):
        ...
        self.topo.command_to(self.topo_config.router,
                             "tcpdump -i {} -s {} -w router.pcap &".format(
                                 #  self.topo_config.get_router_interface_to_client_switch(0),
                                 'any',
                                 snaplen_pcap)
                             )
        ...
        if client_pcap == "yes":
            ...
            for client in self.topo_config.clients:
                self.topo.command_to(
                    client, "tcpdump -i any -w client-{}.pcap &".format(i))
                i += 1

        if server_pcap == "yes":
            i = 0
            for server in self.topo_config.servers:
                self.topo.command_to(
                    server, "tcpdump -i any -w server-{}.pcap &".format(i))
                i += 1
