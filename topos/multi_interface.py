...
class MultiInterfaceTopo(Topo):
    ...
    def __init__(self, topo_builder, parameterFile):
        ...
        # Special case: if there is no specified link between router and server, 
    ...
    # config/topo 내 파일을 통해 topo_parameter로 전달된 path의 특성을 나타냄
...
class MultiInterfaceConfig(TopoConfig):
    NAME = "MultiIf"    # 멤버 변수 NAME에 "MultiIf"를 할당
    ...
    # configure_interfaces 함수를 오버라이딩
    # 각각의 인터페이스에 대한 설정을 수행
    # 구체적인 설정 정보: IP 주소, 서브넷 마스크, ARP 테이블 등을 설정
    ...
    def configure_interfaces(self):
        ...
        # configurations for client to router links
        for i, _ in enumerate(self.topo.c2r_links):
            ...
            self.topo.command_to(
                self.router, f"arp -s {self.get_client_ip(i)} {client_interface_mac}",
            )
        ...
        # configurations for server to router links
